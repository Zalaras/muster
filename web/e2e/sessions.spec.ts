...
// W9-W11. E10/W12 (daemon restart / stale-rail-during-disconnect) get their own serial
// block below with a dedicated daemon, since restarting the shared daemon would break
// every other test in this file running concurrently against it.
//
...
test.describe.serial("daemon restart and disconnect (E10, W12)", () => {
  ...
  test("keeps the stale rail visible during a disconnect and reloads the card from persistence after restart", async ({
    ...
    try {
      ...
      const session = await launchSession(page, restartDaemon, { directory: dir, title: "walk-e10-restart" });
      const claudeId = "claude-e10-1";

      await request.post(restartDaemon.ingestURL("hook"), {
        data: envelopedSessionStart(claudeId, { musterSession: session.id }),
      });
      await request.post(restartDaemon.ingestURL("hook"), { data: rawUserPromptSubmit(claudeId) });

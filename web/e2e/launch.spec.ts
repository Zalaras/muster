...
//
// Recents (GET /api/repos order and count) are asserted precisely by most tests here,
// so — unlike the pre-rebuild file, which shared one daemon across tests — every test
// gets its own scratch daemon (mirrors tiles-launch.spec.ts's `withDaemon`, needed there
// for the same reason: exact global state, not per-test-directory state).
...
test("Cmd+ArrowUp navigates to the parent, and is a no-op at the filesystem root (REQ-6, E5)", async ({ page }) => {
  await withDaemon(async (daemon) => {
    const dir = await browseScratchDirectory(daemon);
    try {
      await page.goto(daemon.dashboardUrl);
...
test("clicking a second recent swaps the pressed mark and the model/mode radios (REQ-7, E6)", async ({ page }) => {
  await withDaemon(async (daemon) => {
...
test("choosing fable launches with model=fable (REQ-9, E10)", async ({ page }) => {
  await withDaemon(async (daemon) => {
    await page.goto(daemon.dashboardUrl);
    const dialog = await openLaunchDialog(page);
...
test("a failed GET /api/repos shows the error and renders the sidebar empty-state (REQ-13)", async ({ page }) => {
  await withDaemon(async (daemon) => {
...
test("the child listing shows No subdirectories for an empty directory (REQ-16)", async ({ page }) => {
  await withDaemon(async (daemon) => {
    const dir = await browseScratchDirectory(daemon);
    try {
      await page.goto(daemon.dashboardUrl);
...
// per-test `withDaemon` and `scratchDirectory` helper. REQ-14 promises every existing
...
test("a recent's title attribute is its full absolute path (REQ-18)", async ({ page }) => {
  await withDaemon(async (daemon) => {
    const dir = await browseScratchDirectory(daemon);
    try {
      await page.goto(daemon.dashboardUrl);
...
test("keyboard traversal: Enter descends into a focused child entry (REQ-15)", async ({ page }) => {
  await withDaemon(async (daemon) => {
    const dir = await browseScratchDirectory(daemon);
    try {

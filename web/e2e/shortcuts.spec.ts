...
//
// AUTHORING MODE: `web/src/shortcuts.ts` and its dispatch from `main.ts`/`render/launch.ts`
// do not exist yet, so every test below is EXPECTED TO FAIL if run — collection-only gate
// for this file. None of these tests pin unchanged behaviour (the chord itself is exactly
// what the plan changes), so none is a regression pin that must run green at authoring.
//
...
//
// Every test gets its own private scratch daemon (mirrors rail-order.spec.ts/views.spec.ts's
// rationale): rail-order and live-tile-membership assertions read full DOM state that a
// concurrently-running test's sessions would corrupt.
 ...
 * its own copy of this fixture rather than sharing one, matching that file's pattern). */
...
test("pressing Opt+Cmd+N in Focus opens the launch dialog (E1)", async ({ page }) => {
...
test("pressing Opt+Cmd+N in Tiles opens the launch dialog (E2, INV-5)", async ({ page }) => {
  await withDaemon(async (daemon) => {
    await page.goto(daemon.dashboardUrl);
...
test("pressing Opt+Cmd+0 in Tiles promotes the neediest session into the grid (INV-5)", async ({ page, request }) => {
  await withDaemon(async (daemon) => {
    const dirs = await Promise.all(Array.from({ length: 5 }, () => scratchDirectory()));
    try {
      await page.goto(daemon.dashboardUrl);
...
test("pressing Opt+Cmd+0 with no sessions is a silent no-op (E7, edge case 3)", async ({ page }) => {
  await withDaemon(async (daemon) => {
    const errors: string[] = [];
...
test("the Focus and Tiles empty placeholders name the new Opt+Cmd+N chord (E9)", async ({ page }) => {
  await withDaemon(async (daemon) => {
    await page.goto(daemon.dashboardUrl);

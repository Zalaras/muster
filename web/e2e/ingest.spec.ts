...
// One scratch daemon per file via fileDaemon(): each test uses its own claude_session_id,
// and the two whole-table counts are before/after deltas within one test (tests on a
// worker's daemon run sequentially), so no test depends on a neighbour's side effects.
...
test("rejects a wrong ingest token with 404 and persists nothing", async ({ page, request }) => {
  ...
  // Prove absence rather than presence — hold past any plausible async persistence.
  await settleFor(page, 300);
...
test("returns 200 for a malformed JSON hook body and persists nothing", async ({ page, request }) => {
  ...
  // Negative check: hold past any plausible async persistence, then prove nothing landed.
  await settleFor(page, 300);

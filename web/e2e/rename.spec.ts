...
//
// `titleOverride` does not exist on the wire, `PUT /api/sessions/{id}/title` does not
// exist, and neither the mainhead heading nor a tile header wraps a rename trigger as of
// this authoring pass — every test in this file is EXPECTED TO FAIL until daemon-impl
// and web-impl land REQ-9 through REQ-16. Collection-only gate for this file.
//
// Every test gets its own private scratch daemon: rename assertions read exact title
// text across the rail/mainhead/tiles/strip, so a session launched by a concurrently
// running test in a shared daemon could corrupt an "untitled" or title-substring match.
...
test("a status-line post's session_name never overrides an active title override (E5)", async ({ page, request }) => {
  await withDaemon(async (daemon) => {
    const { path: dir, cleanup } = await scratchDirectory();
    try {
      await page.goto(daemon.dashboardUrl);
...
test("clearing the field reverts the title to Claude's last-known name (E6)", async ({ page, request }) => {
  await withDaemon(async (daemon) => {
    const { path: dir, cleanup } = await scratchDirectory();
    try {
      await page.goto(daemon.dashboardUrl);
...
test("a header drag still reorders the Tiles grid and opens no rename textbox (E13)", async ({ page }) => {
  await withDaemon(async (daemon) => {
    const [dirA, dirB] = await Promise.all([scratchDirectory(), scratchDirectory()]);
    try {
      await page.goto(daemon.dashboardUrl);
...
test("a rename works on a dead session (REQ-16)", async ({ page }) => {
  await withDaemon(async (daemon) => {
    const { path: dir, cleanup } = await scratchDirectory();
    try {
      await page.goto(daemon.dashboardUrl);

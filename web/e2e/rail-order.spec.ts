...
//
// Every test here gets its own private scratch daemon (mirrors views.spec.ts's
// rationale): rail-order assertions read the FULL `#sessions` DOM order, so a session
// launched by a concurrently-running test in a shared daemon would silently corrupt
// every ordering expectation in this file. `fullyParallel: true` is safe only because
...
test("three launched sessions appear in the rail in creation order with Manual selected (E2)", async ({ page }) => {
...
test("dragging the third card onto the first reorders the rail (E3)", async ({ page }) => {
  await withDaemon(async (daemon) => {
    await page.goto(daemon.dashboardUrl);
...
test("a dragged order persists after a reload (E4)", async ({ page }) => {
  await withDaemon(async (daemon) => {
    await page.goto(daemon.dashboardUrl);
...
test("clicking Pin moves a card to the top of the pinned block (E5)", async ({ page }) => {
  await withDaemon(async (daemon) => {
    await page.goto(daemon.dashboardUrl);
...
test("pinning a second card places it below the first pinned card (E6)", async ({ page }) => {
  await withDaemon(async (daemon) => {
    await page.goto(daemon.dashboardUrl);
...
test("unpinning the first pinned card places it after the remaining pinned card (E7)", async ({ page }) => {
  await withDaemon(async (daemon) => {
    await page.goto(daemon.dashboardUrl);
...
test("dragging an unpinned card onto a pinned card pins it at that position (E8)", async ({ page }) => {
  await withDaemon(async (daemon) => {
    await page.goto(daemon.dashboardUrl);
...
test("a state change in manual mode leaves the rail order unchanged (E9, REQ-7)", async ({ page, request }) => {
  await withDaemon(async (daemon) => {
    await page.goto(daemon.dashboardUrl);
...
test("rail cards are draggable only in Manual mode", async ({ page }) => {
  await withDaemon(async (daemon) => {
    await page.goto(daemon.dashboardUrl);
...
test("the Attention selection persists after a reload (E12)", async ({ page }) => {
  await withDaemon(async (daemon) => {
    await page.goto(daemon.dashboardUrl);
...
test("a second window sees a pin and a reorder without reloading (E13)", async ({ page, browser }) => {
  await withDaemon(async (daemon) => {
    await page.goto(daemon.dashboardUrl);
...
test("the Tiles strip order matches the rail order minus live tiles (E14)", async ({ page }) => {
  ...
  await withDaemon(async (daemon) => {
    ...
    try {
      ...
      expect(railBefore).toEqual(sessions.map((s) => s.id));
...
test("clicking Pin does not change the focused session (E15)", async ({ page }) => {
  await withDaemon(async (daemon) => {
    await page.goto(daemon.dashboardUrl);
...
test("pinning from the Tiles strip pins the session the same as the rail (REQ-13)", async ({ page }) => {
  test.setTimeout(60_000);
  await withDaemon(async (daemon) => {
    await page.goto(daemon.dashboardUrl);
...
test("the rail sort select keeps focus and node identity across a render tick", async ({ page }) => {
  await withDaemon(async (daemon) => {
    await page.goto(daemon.dashboardUrl);

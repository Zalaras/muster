// Plan mermaid-support — the diagram pass: finds `pre > code` mermaid fences inside an
// already-rendered reader body and replaces each with its diagram, or leaves it in place
// with a failure line (REQ-1, REQ-6, REQ-12). Runs strictly after `renderMarkdown` has
// produced the fragment and assigned heading ids (`features/reader.ts` calls this right
// after `setReaderBody`) and touches nothing but mermaid fences — W13.
...
/** Retained per rendered figure, for the theme re-render (REQ-7) — never an attribute,
 * since a diagram's source can be arbitrarily large (Implementation Notes/UI
 * Specifications > DOM). Keyed by the figure so a figure that's later replaced (a fresh
 * `renderDiagrams` pass on a new body) drops its entry for free when garbage collected. */
const sourceByFigure = new WeakMap<Element, string>();

export interface DiagramPassOptions {
  /** A caller-minted value that must be unique to this call (W14: unique per reader
   * instance and per pass) — two instances rendering the same fence (E9: the same file
   * open in two tiles), or two passes of one instance (a fresh open racing a still
   * in-flight one, or a later theme re-render), must never mint the same SVG id, since a
   * mermaid diagram's own `<style>` selects by `#id` and CSS id selectors aren't scoped
   * to one subtree. `features/reader.ts` draws a fresh value from its module counter on
   * every pass, never reusing one across calls. */
  instance: number;
  /** Re-checked before every DOM write (W12) — false once this pass's open has been
...
/**
 * Renders every mermaid fence in `body`, sequentially and in document order (W14) — a
 * fence that fails to parse or render keeps its `<pre><code>` and gains the failure line
 * as its next sibling (REQ-6); everything else in the document, including other
 * diagrams, is untouched. A no-op (returns immediately) when `body` has no mermaid fence
 * — REQ-4's "a document without one loads nothing extra" starts here, before
 * `render/mermaid.ts`'s dynamic import is ever reached.
 */
export async function renderDiagrams(body: HTMLElement, opts: DiagramPassOptions): Promise<void> {
  ...
  for (const code of fences) {
    ...
    // W12: a result for a superseded open, a disposed instance, or a pass a newer one
    // has already overtaken must never touch the DOM.
 ...
 * necessarily the live figure's own SVG. A figure whose source isn't retained (none

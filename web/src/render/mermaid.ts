// Plan mermaid-support — the only module that names mermaid (W5/W11). DOM-only, like
// `reader/markdown.ts` (needs a real `window`, not Vitest-importable —
// `web/src/reader/CLAUDE.md`). Owns the dynamic import (REQ-4), mermaid's own
// configuration (REQ-5, REQ-13) and the DOMPurify pass the SVG crosses before it can ever
...
// Memoised across the whole page — REQ-4: the chunk is requested once, on the first
...
/** The dynamic import — the one line under `web/src` that names the package (W5's grep
 * excludes it: it isn't a static `import … from "mermaid"`). */
function loadEngine(): Promise<MermaidModule> {
  enginePromise ??= import("mermaid").then((mod) => mod.default);
  return enginePromise;
}

/** REQ-13: mermaid's `fontFamily` is the dashboard's own `--sans` stack, read fresh each
...
/** `initialize` is idempotent (Implementation Notes) but re-running it on every render
...
function ensureInitialized(mermaid: MermaidModule, theme: MermaidTheme): void {
  ...
  // REQ-5: strict security level, HTML labels off (root and the deprecated
  ...
  // error SVG suppressed since a parse failure is always caught before render (REQ-6).
  // `layout` is deliberately never set — ELK is 12's default and a diagram's own
  // frontmatter may still ask for it or override it (REQ-2/W23).
  mermaid.initialize({
    startOnLoad: false,
 ...
 * `render` could ever produce mermaid's red error SVG (REQ-6/W18); `render`'s own
 * `bindFunctions` is read from nowhere and therefore never invoked (W11). The SVG string
 * crosses DOMPurify with the `html`+`svg`+`svgFilters` profiles before it becomes a
 * fragment — measured against the installed 12.0.0 output (Implementation Notes): the
...
export async function renderDiagramSvg(
  ...
  // REQ-11: mermaid emits its own `width="100%"` attribute plus an inline
  ...
  // (measured: `figure.scrollWidth === figure.clientWidth`, the E11 regression this
  // fixed). Setting `width`/`height` explicitly, in px, from the `viewBox` is what makes

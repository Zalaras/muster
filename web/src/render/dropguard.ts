// Document-level foreign-drag/drop guard
// (kb:adr/drop-daemon-locates-original-never-stages). Every browser opens a dropped file
// by default unless something calls `preventDefault()` on both `dragover`
// and `drop`; before this guard existed, nothing in `web/src/` did so outside the tile/rail reorder
...
// drag — no MIME sniffing needed here.

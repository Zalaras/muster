...
): Pick<Session, "title" | "titleOverride"> {
...
describe("titleCommand — REQ-14 commit semantics (W5)", () => {
  it("noop: the trimmed input equals the current display title", () => {
    expect(titleCommand("hunting flake", session("hunting flake", "hunting flake"))).toEqual({
      kind: "noop",
    });
  });

  it("noop: trims whitespace before comparing, still equal to the display title", () => {
    expect(titleCommand("  hunting flake  ", session("hunting flake", null))).toEqual({
      kind: "noop",
    });
  });
  ...
  it("clear: whitespace-only input (trims to empty) while an override is currently set (edge case 9)", () => {
    expect(titleCommand("   ", session("hunting flake", "hunting flake"))).toEqual({
      kind: "clear",
    });
  });
  ...
  it("set: any other input sends the trimmed string", () => {
    expect(titleCommand("  new name  ", session("old name", null))).toEqual({
      kind: "set",
      title: "new name",
    });
  });
  ...
  it("set: a first title on a session with no display title yet (null title, no override)", () => {
    expect(titleCommand("first title", session(null, null))).toEqual({
      kind: "set",
      title: "first title",
    });
  });
  ...
  it("set: overwriting one override with a different string", () => {
    expect(titleCommand("second try", session("first try", "first try"))).toEqual({
      kind: "set",
      title: "second try",
    });
  });

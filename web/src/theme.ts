// The web theme registry (kb:adr/theme-pref-enum-follow-not-nullable). The daemon treats
...
/** The Settings-radio guard: every registry name, plus `"follow"`. Derived from THEMES
 ...
 * `"follow"` stays a literal: it is a choice, not a registry member. */
export function isThemeChoice(value: string): value is ThemeChoice {
  return value === "follow" || isThemeName(value);
}

/** The theme resolver: a known theme name wins outright; `"follow"` or any unrecognised
 * string (a renamed/removed theme) resolves by Claude's family —
 * `"light"` -> `"light"`, `"dark"`/`"unknown"` -> `"instrument"`. Pure, so every pref x
 * family cell is covered in Vitest. */
...
/** The first-paint hint shape, mirrored (deliberately duplicated, not imported —
...
/** `features/theme.ts` rewrites this every time it applies a theme or family.

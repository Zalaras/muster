// Breadcrumb path-splitting for the launch dialog's browse pane (plan new-session-dialog,
// REQ-5/REQ-6; review seed B7) — split out of render/crumbs.ts: a DOM-free decision with
// one controller caller, features/launch.ts, lives beside it (docs/conventions.md §
// Composition roots). `render/crumbs.ts`'s `renderCrumbs` stays the DOM half, taking the
// `Crumb[]` this produces as a parameter.

...
// Session.context.
describe("api — fetchReaderListing (GET /api/sessions/{id}/reader, kb:anchor/sessions.reader)", () => {
  let fetchMock: ReturnType<typeof vi.fn>;
...
// The one non-decodeJson call in api.ts (docs/features/reader): a success body is raw
// text, not JSON, so it needs its own Response fake with a .text() method.
function fakeTextResponse(text: string): Response {
  return { ok: true, text: () => Promise.resolve(text) } as unknown as Response;
}

describe("api — fetchReaderFile (GET /api/sessions/{id}/reader/file, kb:anchor/sessions.reader-file)", () => {
  let fetchMock: ReturnType<typeof vi.fn>;

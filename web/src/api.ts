...
export async function fetchReaderListing(id: number): Promise<ApiResult<ReaderListing>> {
  const res = await safeFetch(`/api/sessions/${id}/reader`, { credentials: "same-origin" });
  if (!res) return { ok: false, error: networkError };
  return decodeJson(res, parseReaderListing);
}
 ...
 * A success response is raw `text/markdown` bytes, not JSON — the only `api.ts` call that
 * isn't `decodeJson`; errors still arrive as the usual JSON envelope. Errors:
 * `400 invalid_request` / `404 unknown_session` / `404 not_found` / `413 too_large`. */

...
// Keyed per notice target (WeakMap), not one module-level timer, so two
 ...
 * auto-hide timer first — a new notice (whether it's a fresh outcome replacing an
 * in-flight one, or a second outcome replacing a first) or an explicit clear must never
 * leave a stale timer armed that later fires against unrelated, newer content. */

...
// drag-and-drop feature (kb:adr/drop-daemon-locates-original-never-stages). No DOM, no
// fetch — kept separate from terminal/pane.ts's DOM/socket wiring so it's Vitest-testable
// (docs/conventions.md: "keep logic in pure modules separate from DOM code").
import type { ApiErrorBody } from "../api/http";

/** Terminal.app-style path escaping — a backslash is prefixed to every space and
...
/** A file at or under this size may be uploaded; anything larger is rejected
...
/** Files win whenever any are present, even alongside a
...
/** The in-flight notice — `name` is the dropped basename verbatim, trailing
 * Unicode ellipsis (U+2026). */
export function locatingText(name: string): string {
  return `Locating ${name}…`;
}

/** The exact failure notice text for each outcome — em dash (U+2014) with spaces, matching
 * the terminal overlay texts' glyph. `not_connected` names no file (its notice never
 * mentions a filename), so `name` is ignored for it. */

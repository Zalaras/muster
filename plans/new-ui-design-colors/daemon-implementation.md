...
- **Measured the config file and key against the real, installed pinned binary** (`/Users/bob/.local/share/claude/versions/2.1.258`, `claude --version` → `2.1.258`), per the team lead's explicit ask, read-only:
  - the developer's live `~/.claude.json` has every other top-level scalar key the plan's Edge Case 2 implies ("the developer's own file has no key today") but no `"theme"` key — confirms the "key absent" edge case directly against real data.

...
- **A live interactive surface gets an input round-trip in every view that hosts it**, and the round-trip must span at least one render/update tick (m2-terminal lesson: typing worked in Focus but tiles lost keyboard focus within 1 s of the render tick — the tile round-trip test that would have caught it didn't exist). Asserting the surface *renders* in a view is not evidence it *works* there.

...
supersedes: []
---
**Context.** Several session managers run each agent in a container so that its edits and commands are sandboxed from the host. Muster launches sessions into real checkouts on the developer's machine.
...
**Decision.** B, marked never. The developer's tooling does not all run cleanly in containers, and the resource cost on a laptop running several sessions is unacceptable.

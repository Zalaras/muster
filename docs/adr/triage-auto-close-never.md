...
status: rejected
date: 2026-09-11
summary: A tripwire hit holds an issue for the developer; triage never closes one automatically, since that is an outward write driven by attacker input.
features: [triage]
...
**Options.** (A) Auto-close on a tripwire hit. (B) Hold: the issue is listed for the developer and nothing is written to GitHub.
...
**Consequences.** The tripwire's recall is poor by construction and nothing depends on it. The held list is empty on a normal run. A held issue still counts as untriaged until the developer acts.

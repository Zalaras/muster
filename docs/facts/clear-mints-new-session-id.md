...
guard: TestClearMintsNewSessionID
---
...
Evidence: 2.1.237 probe, 2026-08-20 (capture-3). Guarded since 2026-09-12 by
`TestClearMintsNewSessionID`: run D types `/clear` into its idle window, at no token cost, and
the test asserts the old id on `SessionEnd{clear}`, a different non-empty id on
`SessionStart{clear}`, and the same `tmuxPane` envelope across both

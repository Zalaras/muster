...
no status-line field. Guarded since 2026-09-12 by `TestShiftTabFiresNoHook`: run D sends
`S-Tab` after its `idle_prompt` has already arrived, so the window that follows holds nothing
the keypress did not cause; the test asserts no hook in it and an unchanged status-line key
set. It is an assertion of absence by necessity — that is what the fact claims.

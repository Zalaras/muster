...
C4Container
    ...
    Boundary(agent, "Where sessions run") {
        System_Ext(claude, "Claude Code", "claude CLI, one process per Muster session, plus a version probe at startup and a model-catalog check per model, cached per binary identity, when the launch dialog opens")

...
status: active
date: 2026-08-31
summary: Fix-wave commits shipped with the literal '(review cycle N)' or a guessed number. The orchestrator hands each prompt the exact label; agents copy it.
features: []
tags: [pipeline]
...
**What happened.** Fix-wave agents guessed their commit suffix: two shipped the literal `(review cycle N)`, two more guessed the wrong number, and two pre-review fixes shipped as `(review cycle 1)` because that case had no suffix at all.

**Cost.** A branch history a retro could not read for its cycle count.

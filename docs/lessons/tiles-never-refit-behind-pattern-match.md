...
**What happened.** The tiles view built every terminal into a detached fragment and called fit before attaching it; fit on a zero-size container is a no-op, so no tile ever sent a resize frame. Footers pattern-matched `/\d+×\d+/` and passed on stale geometry, 119×34 shown inside a 597×311 tile. Tiles also lost keyboard focus within a second of the render tick. Five Criticals lived in states the plan's source-state lists omitted — interaction inside the second view.

**Cost.** A review cycle with five Criticals, on a view that rendered correctly and worked nowhere.

**What changed.** A displayed value with an independent oracle is cross-checked against tmux or the API, both sides re-read inside the retry. A live interactive surface gets an input round-trip in every view that hosts it, spanning a render tick. Plans name each hosting view as a source state. A non-protocol requirement is amended mid-run only against a measured defect.

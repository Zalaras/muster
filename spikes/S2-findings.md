...
- **Bash `echo hello` was auto-approved** in default permission mode because this spike deliberately does not isolate `~/.claude/settings.json`, so the developer's personal allowlist applied. The `Write`-tool permission tests were unaffected. Any Muster E2E that depends on a permission prompt must pick a tool the user's allowlist does not cover.

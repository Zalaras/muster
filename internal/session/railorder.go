...
// server package maps it to 400 invalid_request; validation happens before anything is
// computed. applyPin's unknown-id case reuses the existing ErrUnknownSession sentinel
...
// just enough of a Session to decide the invariant, kept as a plain slice so the rebuild
// logic below can be unit tested with no store. Manager's SetPinned/SetOrder extract this
// from the live in-memory registry under its lock, apply here, then persist+broadcast
// only the entries that came back changed.
...
// already has a gap (the only way that happens in production: an earlier Remove is
// allowed to leave one), that renumbering closes the gap and diffChanged reports the
// untouched bystander as "changed" — broadcasting a session the request never named.
...
// nothing is computed on that path.
...
// order) precedes every unpinned one, and railPos becomes the plain 0..n-1 index —
// rebuilding the whole list as pinned block ++ unpinned block rather than clever gap
// arithmetic (kb:adr/rail-order-daemon-owned-per-session-fields). Bystanders may end up
// renumbered too when the invariant requires it; other sessions stay untouched
// otherwise.
...
// Pinned or RailPos differs from before — a no-op call broadcasts nothing.

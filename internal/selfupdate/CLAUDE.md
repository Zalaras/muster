...
**Owns**: resolving `{base}/latest` by redirect, GoReleaser asset naming, `checksums.txt` and its minisign signature, archive download and swap-in, install-kind classification, the cross-process update lock, semver comparison, and composing the one-sentence check/apply failure text a caller displays (`failure.go`'s `DescribeCheckFailure`/`DescribeApplyFailure`) from the typed errors the rest of the package returns. Knows nothing of HTTP handlers, the store or tmux: `internal/server` owns the poller and wire, `cmd/musterd` owns the flag and re-exec (kb:adr/update-release-knowledge-in-selfupdate-package). **Features**: update.
...
**Exemplar**: `release.go` — one bounded network call, a redirect stop that leaves the shared client untouched, typed errors (`TransportError`/`StatusError`/`TagError`) classified by `failure.go` into the one-sentence UI text rather than string-matched; their own `Error()` text is for the log only.

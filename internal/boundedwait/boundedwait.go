...
// package boundaries: bounded-wait-with-warn — "cancel (or close), wait for the goroutine
// to exit, but give up and warn once ctx is done", needed by internal/session's liveness
// poll and by internal/server's ingest queue, update apply and background loop (bgloop.go,
// which every poller runs on). internal/session must
// never import internal/server (kb:diagram/daemon-components), so the shared half lives
// here instead, importing nothing internal itself. Named for the wait, not "lifecycle" —

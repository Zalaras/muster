...
// token — those all change across daemon instances/restarts, but the shape doesn't: a
// naive "matches cfg.HookURL exactly" check would fail to recognize Muster's own stale
// entry once the port/token rotates, and the wholesale-replace guarantee depends on
// recognizing it anyway.
